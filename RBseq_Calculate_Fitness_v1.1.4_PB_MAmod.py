...
Version = '1.1.4_PB'
ReleaseDate = 'Sept 1, 2020'
# poolCount file needs to be sorted by scaffold and position - but if largest scaffold is not
# at top of list you get error, so reset_index fixes the problem
# scaffData = strainFit[strainFit['scaffold'] == longestScaffold].reset_index(drop=True)

# statusUpdate = 'Loaded counts for ' + str(len(poolCounts)-1) + ' barcodes from ' + poolCountFiles[0]
# should be str(len(poolCounts))
...
def main(argv):
    ...
    for replicateGroup in groupSet:
        if False in pairedFlags[groupSampNumbers[replicateGroup]]:
            ...
            group_references=references[groupSampNumbers[replicateGroup]] #MA mod 1/6/2021: drop rows with no references when averaging the references to avoid an error from missing data
            group_references=group_references[~pd.isnull(group_references)]
            poolCounts[newColumn] = poolCounts[group_references].mean(axis=1)
            for sampNum in groupSampNumbers[replicateGroup]:
                references[sampNum] = newColumn
    

    # statusUpdate = 'Loaded counts for ' + str(len(poolCounts)-1) + ' barcodes from ' + poolCountFiles[0]
    statusUpdate = 'Loaded counts for ' + str(len(poolCounts)) + ' barcodes from ' + poolCountFiles[0]
    printUpdate(options.logFile,statusUpdate)
