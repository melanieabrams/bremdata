...
def calc_mean_insert_ratios(grp):
        grp['mean39_28'] = np.nanmean(grp['39_28_log2_br_averaged_reads']) 
	#grp['mean39_28'] = np.mean(grp['39_28_log2_br_averaged_reads']) # calculate the geometric mean of each allele's ratios
	return grp

def calc_stdev_insert_ratios(grp):
        grp['stdev9_28'] = np.nanstd(grp['39_28_log2_br_averaged_reads']) # calculate the geometric mean of each allele's ratios
	#grp['stdev9_28'] = np.std(grp['39_28_log2_br_averaged_reads']) # calculate the geometric mean of each allele's ratios
	return grp
...
if __name__ == '__main__':
	...
	for file_identifier, each_df in file_id_dict.iteritems():
		...
		for index, row in grouped.iterrows():
			...
			#row_mean = np.mean(row['39_br_averaged_reads']/row['28_br_averaged_reads'])
			row_mean = np.nanmean(row['39_br_averaged_reads']/row['28_br_averaged_reads'])
		...
		for each_gene, gene_info in gene_dict.iteritems():
			...
			cv_sc = np.absolute(np.std(gene_info.sc_ratios)/np.nanmean(gene_info.sc_ratios)) #changed to nanmean
			cv_sp = np.absolute(np.std(gene_info.sp_ratios)/np.nanmean(gene_info.sc_ratios)) #changed to nanmean
