...
### USAGE ###
# finally, computes T-TEST (vs. Carly's Mann-Whitney U test), compares the log2(39/28) values of inserts in the Scer allele vs the Spar allele
...
if __name__ == '__main__':
	...
	for file_identifier, each_df in file_id_dict.iteritems():
		...
		for each_gene in unique_genes:
			...
			t_test = stats.ttest_ind(sc_inserts_ratios, sp_inserts_ratios) # independent t-test
			#mwu_test = stats.mannwhitneyu(sc_inserts_ratios, sp_inserts_ratios)   # mwu test
			#two_tailed_pval = mwu_test[1] * 2.0
			gene_object.t_statistic = t_test[0]
			gene_object.two_tailed_pval = t_test[1]
		...
		pval_df.to_csv(str(output_folder)+str('/')+str(file_identifier)+'.t_test_results', sep='\t', index=False)
