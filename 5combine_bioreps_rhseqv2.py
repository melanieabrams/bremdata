...
# combines biological replicates into one file, e.g. ctrlA, ctrlB, ctrlC into ctrl
...
def parse_file(filename, sep='\t'): 
        ...
        if file_identifier == 'T0':
                ...
                condition_ID = file_identifier[:-1]  # chops off the last character, e.g. exptlC becomes exptl
...
if __name__ == '__main__':
        ...
        for each_condition, list_of_df in condition_df_dict.items():
                global test
                test = 0.0
                ...
                if each_condition == 'T0':
                        ...
                        idx_df.index.name = None #added this line for py2.7 from 2.6
                        ...
                        idx_grouped.index.name = None #added this line for py2.7 from 2.6
                        ...
                        merged.replace(to_replace=0.0, value=np.nan, inplace=True) #replaces 0s with np.nan for calculations'
                        columns_to_average = [column for column in list(merged.columns.values) if 'n_av' in column]
                        merged['br_averaged_reads'] = merged[columns_to_average].mean(axis=1,skipna=True) #ignore nans
                        ...
                        merged.replace(to_replace=np.nan, value='None', inplace=True) #replaces nan with 'None so file can be still parsed in the next script'
