...
cutoff_gene_cv_ratio = 1.75 #coefficient of variation of the ratio of 39/28 for an insert
min_sp_logratio=-0.75
max_sp_logratio=0.75
...
class GeneObject(object):
	...
	def __init__(self, gene_name):
		...
		self.sc_ratios = []
		self.sp_ratios = []
		self.sp_mean_logratio=np.inf
		self.cv={'sp':np.nan,'sc':np.nan}
...
def getCV(gene,allele,gene_dict):
...
if __name__ == '__main__':
	...
	for file_identifier, each_df in file_id_dict.iteritems():
		...
		each_df.drop(columns=['39_28_log2_br_coeffvar'])
		grouped=each_df #renamed here for consistency across the rest of the script, different operations from original rh-seq pipeline
		...
		grouped['39_avg_read/28_avg_reads']=grouped['39_br_averaged_reads']/grouped['28_br_averaged_reads']
		for index, row in grouped.iterrows():
			...
			row_mean=row['39_avg_read/28_avg_reads']
			...
			if row_allele == 'sp':
				gene_object.number_sp_inserts += 1.0
                                ...
                                gene_object.sp_mean_logratio=row['39_28_log2_br_averaged_reads'] #added in v3
...
##                                print cv_sc
			...
			if number_sc >= number_inserts_per_allele_needed_to_test and number_sp >= number_inserts_per_allele_needed_to_test:
                                if cv_sc <= cutoff_gene_cv_ratio and cv_sp <=cutoff_gene_cv_ratio:
                                        if min_sp_logratio<gene_info.sp_mean_logratio<max_sp_logratio:
                ...
                print('...adding cvs to dataframe...')

                grouped['cv_39_avg_read/28_avg_reads']=grouped.apply(lambda x: getCV(x['gene'],x['allele'],gene_dict),axis=1) #add cv to df

##                for index,row in grouped.iterrows():
##                        row_gene_name=row['gene']
##                        row_allele=row['allele']
##                        row_cv=getCV(row_gene_name,row_allele,gene_dict)
##                        grouped.at[index,'cv_39_avg_read/28_avg_reads']=row_cv
		...
		print 'sp centered out of range: '+str(sp_out_of_range)

                filtered_gene_df.to_csv(str(output_folder)+str('/')+str(file_identifier)+'_ratiocvfilt_'+str(cutoff_gene_cv_ratio)+'_'+str(number_inserts_per_allele_needed_to_test)+'_min'+str(max_sp_logratio).replace('.','pt')+'max'+str(max_sp_logratio).replace('.','pt')+'splog.filtered_gene_inserts', sep='\t', index=False)
