    ...
    for sampleName in allCounts:
        ...
        try:
            with open(fileToOpen, 'w') as file:
                json_string=json.dumps(allCounts[sampleName])
                sampCount_df=pd.DataFrame.from_dict(json.loads(json_string),orient='index')
                sampCount_df=sampCount_df.rename(columns={0: 'barcode', 1:'count'})
                sampCount_df.to_csv(fileToOpen,sep="\t")
...
##                file.write(json.dumps(allCounts[sampleName]))
    ...
    statusUpdate = "Compiling counts for barcodes, with no poolFile in this script version"
    printUpdate(options.logFile,statusUpdate)
