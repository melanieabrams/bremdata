...
if __name__ == '__main__':
	...
	for file_identifier, each_df in file_id_dict.iteritems():
		...
		for index, row in grouped.iterrows():
			...
			row_mean = np.mean(row['39_28_log2_br_averaged_reads'])
 ...
 =;j			number_sp = float(gene_info.number_sp_inserts)
			stdev_sc = np.absolute(np.std(gene_info.sc_ratios))
			stdev_sp = np.absolute(np.std(gene_info.sp_ratios))
			if number_sc >= number_inserts_per_allele_needed_to_test and number_sp >= number_inserts_per_allele_needed_to_test:
                                if stdev_sc <= cutoff_gene_stdev and stdev_sp <=cutoff_gene_stdev:
		...
		filtered_gene_df.to_csv(str(output_folder)+str('/')+str(file_identifier)+'_stdevfilt_'+str(cutoff_gene_stdev)+'_'+str(number_inserts_per_allele_needed_to_test)+'.filtered_gene_inserts', sep='\t', index=False)
