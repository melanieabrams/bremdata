...
#python3 9fisher.py outdir file(s)

# finally, computes fisher test with average 39, 28 values of inserts in the Scer allele vs the Spar allele
# test is two-tailed (want to see if scer > spar or spar > scer), by default
...
if __name__ == '__main__':
	...
	for file_identifier, each_df in file_id_dict.items():
		...
		for each_gene in unique_genes:
			...
			sc_28=np.mean(sc_df['28_br_averaged_reads'].tolist())
			sp_28=np.mean(sp_df['28_br_averaged_reads'].tolist())
			sc_39=np.mean(sc_df['39_br_averaged_reads'].tolist())
			sp_39=np.mean(sp_df['39_br_averaged_reads'].tolist())
			oddsratio, two_sided_pval=stats.fisher_exact([[sc_28, sc_39],[sp_28,sp_39]]) #fisher test
			gene_object.oddsratio = oddsratio
			gene_object.two_sided_pval = two_sided_pval
			unique_genes_dict[each_gene] = gene_object
			pval_dict[each_gene] = two_sided_pval
		...
		pval_df.to_csv(str(output_folder)+str('/')+str(file_identifier)+'.fisher_test_results', sep='\t', index=False)
