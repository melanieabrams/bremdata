...
def make_plots(gene_df):
        ...
        for biorep in sc_bioreps:
            sc_ratios+=sc_df[biorep].stack().groupby(level=0).apply(list).tolist() #stacking drops nans
            print(sc_ratios)
            #sc_ratios+=sc_df[biorep].tolist()
        for biorep in sp_bioreps:
            sp_ratios+=sp_df[biorep].stack().groupby(level=0).apply(list).tolist()
...
##			sp_inserts_ratios = [float(i) for i in sp_df['39_28_log2'].tolist()]
                    ...
                    sp_bioreps = [col for col in sp_columns if col.startswith('39_28_log2') and col.endswith('_n_av')]
                    for biorep in sc_bioreps:
                        sc_inserts_ratios+=np.mean(sc_df[biorep].tolist()) #recollapse here
                    for biorep in sp_bioreps:
                        sp_inserts_ratios+=np.mean(sp_df[biorep].tolist())       #and here (mean of all ins for biorep, not each one alone)
