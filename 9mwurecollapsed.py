...
#this is supposed to recollapsed the uncollapsed-bioreps which have been filtered for stdev across inserts
...
def make_plots(gene_df):
	...
	sc_ratios = sc_df['39_28_log2_br_averaged_reads'].tolist()
	sp_ratios = sp_df['39_28_log2_br_averaged_reads'].tolist()
...
if __name__ == '__main__':
	...
	for file_identifier, each_df in file_id_dict.items():
		...
		for each_gene in unique_genes:
			...
			sc_inserts_ratios = [float(i) for i in sc_df['39_28_log2_br_averaged_reads'].tolist()]
			sp_inserts_ratios = [float(i) for i in sp_df['39_28_log2_br_averaged_reads'].tolist()]
		...
		pval_df.to_csv(str(output_folder)+str('/')+str(file_identifier)+'_recollapsed.mwu_test_results', sep='\t', index=False)
