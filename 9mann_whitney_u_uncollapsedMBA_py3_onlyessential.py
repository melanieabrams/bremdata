...
#Parameters:
essential_genes="/usr2/people/mabrams/Amended_Genomes/deletion_project_Essential_ORFs.txt"
...
def parse_essential_genes(essential_textfile):
        '''parse the essential gene list from deletion collection'''
        ynames=[]
        with open(essential_textfile) as f:
            for line in f:
                row_data=line.split('\t')
                if len(row_data)>1 and len(row_data[0])>1:
                    yname=row_data[1].strip(' ')
                    if yname[0]=='Y':
                        ynames.append(yname)
        return ynames
...
def make_plots(gene_df):
        ...
        for biorep in sc_bioreps:
            sc_ratios+=sc_df[biorep].tolist()
        for biorep in sp_bioreps:
            sp_ratios+=sp_df[biorep].tolist()
    ##	sc_ratios = sc_df['39_28_log2'].tolist() UNCOLLAPSED THIS 
...
if __name__ == '__main__':
	...
	files = argv[2:]
	essential_list=parse_essential_genes(essential_genes)
	...
	for file_identifier, each_df in file_id_dict.items():
		all_unique_genes = each_df.gene.unique()
		essential_unique_genes=[]
		for gene in all_unique_genes:
                        if gene in essential_list:
                                essential_unique_genes.append(gene)
		
		unique_genes_dict = {key: None for key in essential_unique_genes}
		another_gene_dict = {key: None for key in essential_unique_genes}
		pval_dict = {key: None for key in essential_unique_genes}
	
		for each_gene in essential_unique_genes:
...
##                    if each_gene == "YHR034C":  #Test a given gene
##                            print(each_gene)
...
##                                    print(i)
		...
		pval_df.to_csv(str(output_folder)+str('/')+str(file_identifier)+'_onlyessential.mwu_test_results', sep='\t', index=False)
