...
def PlotDensity(spar_ins, scer_ins,rep_columns,geneName):
    '''only plotting the densities of bioreps with a distr for both alleles here (hardcoded into multipanel)'''
    figName=geneName+"_py_density_separate_replicates_repsinbothalleles.pdf"

    fig, axs = plt.subplots(2,4, figsize=(20, 10))
    fig.delaxes(axs[1,3]) #remove the empty 8th
    fig.subplots_adjust(hspace = .3, wspace=.3)
    fig.suptitle(geneName)
    plt.rc('xtick',labelsize=10)
    plt.rc('ytick',labelsize=10)

    rep_columns_with_both=['39_28_log2A_n_av', '39_28_log2B_n_av','39_28_log2D_n_av', 
       '39_28_log2H_n_av', '39_28_log2I_n_av', '39_28_log2K_n_av', '39_28_log2M_n_av']


    for ax, rep_col in zip(axs.ravel(), rep_columns_with_both):
        ...
        print(rep_col[10])
        print(scer_ins[rep_col],spar_ins[rep_col])
