...
### USAGE ###
# python convert_poolcount_to_fastqpooledreadsclean_rhseqv2.py output_folder filtered_insert_ratio_file.poolcounts 
#converts poolcount file format from rbseq pipeline into .fastq_pooled_read_clean format for downstream rh-seq pipeline
...
def build_rename_dict(df,br_columns):
    ...
    ctrlA1 - biorep A of control temp, tech rep 1
    exptlA1 - birep A of test temp, tech rep 1'''
    ...
    alphabet=alphabet.replace('E','') #remove E, because bioreps named E introduce a bug downstream
    
    #generate renamin dictionary
    rename_dict={}
    for i in range(len(control_brs)):
        rename_dict[control_brs[i]]='ctrl'+alphabet[i]+'1.fastq_pooled_reads_clean'
    for i in range(len(test_brs)):
        rename_dict[test_brs[i]]='exptl'+alphabet[i]+'1.fastq_pooled_reads_clean'
