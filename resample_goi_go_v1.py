...
def resampleAllGo(go_term_groups,goi,go_terms,essential_count,non_essential_count,n=10000,save_intermediate=True):
    ...
    samples=[]
    for i in range(n):
        sample = essential_go_df.sample(n=essential_count,replace=True)
        sample = sample.append(non_essential_go_df.sample(n=non_essential_count,replace=True))
...
##                print(n_go_goi,n_go_sample)
        ...
        rv_dict[term]=[n_samples_greater_or_equal_to_goi,n_go_goi,median_random]
        


##    print(rv_dict['GO:0005737'])
    rv_df=pd.DataFrame.from_dict(rv_dict, orient='index',columns=['rv','count_in_goi','median_count_in_random_sample'])
    rv_df.to_csv('resample_goi_go_df.tsv', sep='\t', index=True)
    print(rv_df)
