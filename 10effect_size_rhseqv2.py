...
def FilterEffectSize(effect_dict, mean_dict,stats_dict,p_cutoff,save_name='filtered_effect_sizes.txt',printTop=4):
    ...
    if save_fx_size==True:
        print('...saving outfile:')
        wf=open(save_name,'w')
        wf.writelines('gene\teffect_size\tfitter_allele\tsc_mean\tsp_mean\tmwu_pval\n')
        for gene in sorted(filtered_effect_dict, key=filtered_effect_dict.get, reverse=True):
            wf.writelines(gene +'\t'+str(filtered_effect_dict[gene])+'\t'+dir_dict[gene]+'\t'+str(mean_dict[gene][0])+'\t'+str(mean_dict[gene][1])+'\t'+str(stats_dict[gene])+'\n')
        wf.close()
